...
`new`是一个运算符，它调用了`operator new`函数来分配内存，然后**调用了类的构造函数来创建对象**
...
2. 进程之间相互独立，一个进程崩溃不会影响其他进程，而线程之间共享进程的地址空间，一个线程崩溃可能导致整个进程崩溃。
3. 进程通信需要通过IPC机制，如管道、消息队列、信号量等，而线程可以通过共享内存等更简单的方式进行通信。
4. 创建、撤销进程的开销较大，而线程的创建、撤销开销较小。
...
point2D的点对象->point3D 对获得的三维点通过三维空间的矩阵几何变换实现其在曲面上的投影计算,本质是点数据在不同坐标系下的转换计算，确定曲面上的质检点，一方面需要计算点转换后的坐标系下的具体坐标值，同时也需要确定该点的法向向量数值;

最小生成树：

生成树的定义：一个连通图的生成树就是指这个图的一个连通子图，对于这个连通图的子图必须包含了图中所有的顶点，但是这个图的边的数量为其顶点数量减一，即生成树为有N个顶点但只有N-1条边的连通图，如果这棵生成树再添加任意一条边就会形成环。

最小生成树：就是在连通图的所有生成树中，其所有边的权值之和加起来最小的那棵生成树
...
#include <queue>
using namespace std;
int minBroadcastStations(vector<vector<char>>& matrix) {
    int n = matrix.size();
...
int main() {
    ...
    cout << minBroadcastStations(matrix) << endl;
    return 0;
}
```

...
`argc`和`argv`是C和C++程序中的标准参数，这些参数传递给程序的`main()`函数。
...
`const int fun()` 表示返回一个常量整数值。如果该函数返回一个值时，返回的值会被视为常量不能被修改。
...
![IMG_256](https://gitee.com/juzihhu/image_bed/raw/master/img/202211042209246.jpg)
...
重载指的是在同一个类中，有多个同名方法，但是这些方法的参数列表不同。通过重载，可以让同一个方法名在不同情况下执行不同的操作。
...
public:
    ...
    // 如果没有声明为virtual  sizeof该类对象占1字节内存
    // virtual 4字节（虚函数表指针）记录虚函数地址
    virtual void speak()
    {
        cout << "动物在说话" << endl;
    }
...
![](https://gitee.com/juzihhu/image_bed/raw/master/img/202211042209247.png)

**注意：**

1. **假如子类B和子类C中不存在虚函数，那么这时他们将共用基类A的一张虚函数表**;当子类中有虚函数时他们就需要各自产生一张虚函数表，并用各自的虚指针指向该表。由于子类对 `speak`作了重载，因此他们有三种不同的实现方式，函数地址也不尽相同，在使用的时候需要从各自类的虚函数表中去查找对应的 `speak` 地址。

2. 对于虚函数 *eat*，子类都没有进行重载操作，所以基类和子类将共用一个 *eat*，该虚函数的地址会分别保存在两个类的虚函数表中，但他们的地址是相同的；

3. 在使用`animal`调用 *speak* 的时候，程序发现`animal`是一个指针，并且现在正在调用一个虚函数叫做 `speak`，这时通过 *animal->vptr* 这个虚指针到类Cat的虚函数中找对应的虚函数地址，找到该地址以后，就用相应的虚函数来进行调用

4. 如何从虚函数表中查找到 *vfunc1* 的地址？
...
C++虚函数是一种特殊的成员函数，用于实现运行时多态。在C++中，当使用一个指向基类对象的指针或引用来调用一个虚函数时，实际上会根据对象的实际类型来调用相应的派生类中的虚函数，而不是调用基类中的虚函数。
...
![1545985945198](https://gitee.com/juzihhu/image_bed/raw/master/img/202211042209249.png)**示例：**
...
// 纯虚析构既要有声明，也要有实现（基类对象也可能在堆区开辟内存）
// 基类的虚函数可以只声明
Animal::~Animal()
{
...
![image-20220810202044713](https://gitee.com/juzihhu/image_bed/raw/master/img/202211042209250.png)
...
总结：类模板中成员函数类外实现时，需要加上**模板参数列表**`Person<T1, T2>::`
...
public:
    ...
    MyArray(const MyArray & arr)
    {
        ...
        //this->pAddress= arr.pAddress; // 若指针这样简单赋值，会发生浅拷贝问题
...
void test01() {
    ...
    //类似回调技术 MyPrint 底层实现用for循环调用该函数
...
![说明: 2015-11-10_151152](https://gitee.com/juzihhu/image_bed/raw/master/img/202211042209251.jpg)
...
![说明: 2015-11-19_204101](https://gitee.com/juzihhu/image_bed/raw/master/img/202211042209252.jpg)
...
![clip_image002-1547547896341](https://gitee.com/juzihhu/image_bed/raw/master/img/202211042209253.jpg)
...
![说明: 2015-11-15_195707](https://gitee.com/juzihhu/image_bed/raw/master/img/202211042209254.jpg)
...
![img](https://gitee.com/juzihhu/image_bed/raw/master/img/202211042209255.png)

![img](https://gitee.com/juzihhu/image_bed/raw/master/img/202211042209256.jpg)
...
![说明: 2015-11-15_214429](https://gitee.com/juzihhu/image_bed/raw/master/img/202211042209257.jpg)
...
![1547606785041](https://gitee.com/juzihhu/image_bed/raw/master/img/202211042209258.png)
...
![说明: 2015-11-15_225145](https://gitee.com/juzihhu/image_bed/raw/master/img/202211042209259.jpg)
...
void test01()
{
    ...
    //排序
    // 所有不支持随机访问迭代器的容器，不可以用标准算法 sort(L.begin(),L.end());
    // 可以使用其内部提供的算法
    L.sort(); //默认的排序规则 从小到大
    printList(L);
    //利用函数或者仿函数改变排序规则
...
int main() {
    ...
    for_each(v.cbegin(), v.cend(), print), cout << "\n";
